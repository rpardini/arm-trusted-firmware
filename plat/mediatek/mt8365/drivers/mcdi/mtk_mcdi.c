...
{
	struct mcupm_fw_header *hdr;
	char *addr;

	hdr = (struct mcupm_fw_header *) mcdi_fw_base_addr;

	while (hdr->magic == MCUPM_FW_MAGIC) {
		if (!strcmp(hdr->name, name))
			break;

		hdr = (struct mcupm_fw_header *) (((char *) hdr) +
						sizeof(*hdr) + hdr->size);
	}

	if (hdr->magic == MCUPM_FW_MAGIC) {
		addr = (((char *) hdr) + sizeof(*hdr));
		return addr;
	}
