...
static const struct power_domain_data power_domain_data_mt8195[MT8195_POWER_DOMAIN_NR] = {
...
{
	const struct power_domain_data *pd = &power_domain_data_mt8195[ID];
...
void spm_mtcmos_on_all(void)
{
	...
	for (i = 0; i < MT8195_POWER_DOMAIN_NR; i++)
		spm_mtcmos_on(i);
}
