...
static void __pwrap_spi_clk_set(void)
{
	...
	WRAP_WR32(CLK_CFG_5_CLR, 0x93000000);
	WRAP_WR32(CLK_CFG_5_SET, CLK_SPI_CK_26M << 24);
	WRAP_WR32(CLK_CFG_UPDATE, 0x1 << 23);
#endif
...
#ifdef DUAL_PMICS
	pwrap_write_nochk(EXT_SPISLV_KEY, 0x6358);
#endif
...
static void _pwrap_starve_set(void)
{
	...
	WRAP_WR32(PMIC_WRAP_STARV_COUNTER_16, 0x740);
...
static void _pwrap_enable(void)
{
	...
	WRAP_WR32(PMIC_WRAP_INT1_EN, 0xeef7ffff); /* disable Matching interrupt for bit 28 */
}
...
{
	unsigned int rdata;
	int si_en_sel, si_ck_sel, si_dly, si_sample_ctrl, clk_edge_no;
	...
	for (si_en_sel = 0; si_en_sel < 8; si_en_sel++) {
		for (si_ck_sel = 0; si_ck_sel < 2; si_ck_sel++) {
			...
			if (rdata == DEFAULT_VALUE_READ_TEST) {
				...
				PWRAPCRI("found si_en_sel = %x, si_ck_sel = %x, si_sample_ctrl = %x, rdata = %x\n",
				         si_en_sel, si_ck_sel, si_sample_ctrl, rdata);
				found = 1;
				break;
			}
			PWRAPCRI("si_en_sel = %x, si_ck_sel = %x, *PMIC_WRAP_SI_SAMPLE_CTRL = %x, rdata = %x\n",
			         si_en_sel, si_ck_sel, si_sample_ctrl, rdata);
		}
		if (found == 1)
			break;
	...
	pwrap_read_nochk(PMIC_DEW_READ_TEST_ADDR, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPCRI("_pwrap_init_sistrobe Read Test Failed, rdata = %x, exp = 0x5aa5\n", rdata);
		return 0x10;
	}
...
static int __pwrap_InitSPISLV(void)
{
	...
	pwrap_write_nochk(MT6358_DRV_CON1, 0x8888); /* set PMIC GPIO driving current to 4mA */
#ifdef DUAL_PMICS
...
#define PWRAP_GPIO_DRIVING_ADDR  0x11F200A0
	...
	*/
	...
	PWRAPCRI("RECORD_CMD0:0x%x (The last command addr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA0_ADDR, &rdata);
	PWRAPCRI("RECORD_WDATA0:0x%x (The last command wdata)\n", rdata);
	pwrap_read_nochk(PMIC_RECORD_CMD1_ADDR, &rdata);
	PWRAPCRI("RECORD_CMD1:0x%x (The second-last command addr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA1_ADDR, &rdata);
	PWRAPCRI("RECORD_WDATA1:0x%x (The second-last command wdata)\n", rdata);
	pwrap_read_nochk(PMIC_RECORD_CMD2_ADDR, &rdata);
	PWRAPCRI("RECORD_CMD2:0x%x (The third-last command addr)\n", (rdata & 0x3fff));
	pwrap_read_nochk(PMIC_RECORD_WDATA2_ADDR, &rdata);
	PWRAPCRI("RECORD_WDATA2:0x%x (The third-last command wdata)\n", rdata);
	...
	PWRAPLOG("GPS_CMD_ADDR:0x%x, GPS_CMD:0x%x, GPS_RDATA_ADDR:0x%x\n",
		WRAP_RD32(PMIC_WRAP_INT_GPS_AUXADC_CMD_ADDR),
		WRAP_RD32(PMIC_WRAP_INT_GPS_AUXADC_CMD),
		WRAP_RD32(PMIC_WRAP_INT_GPS_AUXADC_RDATA_ADDR));
	PWRAPLOG("GPSINF_0_STA:0x%x, GPSINF_1_STA:0x%x\n",
		WRAP_RD32(PMIC_WRAP_GPSINF_0_STA),
		WRAP_RD32(PMIC_WRAP_GPSINF_1_STA));
	...
	/* enable Matching interrupt for bit 28 */
	WRAP_WR32(PMIC_WRAP_INT1_EN, WRAP_RD32(PMIC_WRAP_INT1_EN) | 0x10000000);
