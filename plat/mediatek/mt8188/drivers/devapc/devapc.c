...
static const struct apc_infra_peri_dom_16 peri2_ao_sys0_devices[] = {
	...
	DAPC_PERI2_AO_SYS0_ATTR("BND_NORTH_APB2_S",
				NO_PROTECTION, FORBIDDEN, FORBIDDEN, FORBIDDEN, FORBIDDEN,
	...
	for (d = 0; d < DOM_NUM_INFRA_AO_SYS1; d++)
		for (i = 0; i < reg_max; i++)
			INFO("[DEVAPC] (INFRA_AO_SYS1)D%d_APC_%d: 0x%x\n", d,
			       i, mmio_read_32(getreg_domain(base, SYS1_D0_APC_0, d, i)));
	...
	for (d = 0; d < DOM_NUM_INFRA_AO_SYS2; d++)
		for (i = 0; i < reg_max; i++)
			INFO("[DEVAPC] (INFRA_AO_SYS2)D%d_APC_%d: 0x%x\n", d,
			       i, mmio_read_32(getreg_domain(base, SYS2_D0_APC_0, d, i)));
...
{
	/* Side-band */
	mmio_clrsetbits_32(getreg(base, MAS_SEC_0), MCUPM_SEC, SECURE_TRANS << 1);
...
MTK_PLAT_SETUP_0_INIT(dapc_init);
