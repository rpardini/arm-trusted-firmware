...
int spm_update_rc_bus26m(int state_id, int type, const void *val)
{
	...
	if (type == PLAT_RC_UPDATE_CONDITION) {
	...
	} else if ((type == PLAT_RC_IS_FMAUDIO) || (type == PLAT_RC_IS_ADSP)) {
		int *flag = (int *)val;
		...
		if (*flag != 0) {
			bus26m_ext_opand2 |= MT_SPM_EX_OP_PERI_ON;
		} else {
			bus26m_ext_opand2 &= ~MT_SPM_EX_OP_PERI_ON;
		}
		...
		if (*flag != 0) {
			bus26m_ext_opand2 |= MT_SPM_EX_OP_INFRA_ON;
		} else {
			bus26m_ext_opand2 &= ~MT_SPM_EX_OP_INFRA_ON;
		}
...
int spm_reset_rc_bus26m(unsigned int cpu, int state_id)
{
	...
	if (IS_PLAT_SUSPEND_ID(state_id)) {
		...
		if (spm_unlikely(status.is_valid & MT_SPM_RC_VALID_RESIDNECY)) {
			status.residency += waken ? waken->tr.comm.timer_out : 0;
		}
	}
