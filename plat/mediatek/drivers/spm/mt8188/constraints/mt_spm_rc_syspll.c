			     ...
			     unsigned int *resource_req)
{
	...
	spm_lp->pwrctrl->pcm_flags =
				(uint32_t)CONSTRAINT_SYSPLL_PCM_FLAG;
	spm_lp->pwrctrl->pcm_flags1 =
				(uint32_t)CONSTRAINT_SYSPLL_PCM_FLAG1;
	...
	if (type == PLAT_RC_UPDATE_CONDITION) {
		...
		if (is_ufs_h8)
			...
			spm_rc_constraint_valid_clr(MT_RM_CONSTRAINT_ID_SYSPLL,
				    ...
				    MT_SPM_RC_VALID_UFS_H8,
				   (struct constraint_status * const)&status);
	} else if (type == PLAT_RC_IS_USB_PERI) {
		int *flag = (int *)val;

		if (*flag)
			syspll_ext_opand2 |= MT_SPM_EX_OP_PERI_ON;
		else
			syspll_ext_opand2 &= ~MT_SPM_EX_OP_PERI_ON;
	} else if (type == PLAT_RC_IS_USB_INFRA) {
		int *flag = (int *)val;
		...
		do {
			...
			if (st->type == CONSTRAINT_UPDATE_COND_CHECK) {
				...
				spm_rc_constraint_status_set(st->id, st->type,
				     st->act, MT_RM_CONSTRAINT_ID_SYSPLL,
