...
int mt_spm_suspend_mode_set(int mode, void *prv)
{
	if (mode == MT_SPM_SUSPEND_SLEEP) {
		suspend_ctrl.pcm_flags =
				SPM_SUSPEND_SLEEP_PCM_FLAG;
		suspend_ctrl.pcm_flags1 =
				SPM_SUSPEND_SLEEP_PCM_FLAG1;
	} else {
		suspend_ctrl.pcm_flags =
				SPM_SUSPEND_PCM_FLAG;
		suspend_ctrl.pcm_flags1 =
				SPM_SUSPEND_PCM_FLAG1;
	}
	return 0;
}

int mt_spm_suspend_enter(int state_id,
			 unsigned int ext_opand, unsigned int reosuce_req)
{
	int ret = 0;
	...
	if (ext_opand & MT_SPM_EX_OP_PERI_ON)
		suspend_ctrl.pcm_flags |= SPM_FLAG_PERI_ON_IN_SUSPEND;
	else
		suspend_ctrl.pcm_flags &= ~SPM_FLAG_PERI_ON_IN_SUSPEND;

	if (ext_opand & MT_SPM_EX_OP_INFRA_ON)
