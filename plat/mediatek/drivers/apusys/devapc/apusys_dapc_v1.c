...
int32_t __set_apusys_dapc_v1(const struct apc_dom_16 *dapc, uint32_t size, dapc_cfg_func cfg)
{
...
void __dump_apusys_dapc_v1(const char *name, uintptr_t base, uint32_t reg_num, uint32_t dom_num)
{
	uint32_t d, i;
	...
	for (d = 0; d < dom_num; d++)
		for (i = 0; i <= reg_num; i++)
			INFO("[%s] D%d_APC_%d: 0x%x\n", name, d, i,
					apuapc_readl(base +	d * 0x40 + i * 4)
			);

	INFO("[%s] APC_CON: 0x%x\n", name,
		apuapc_readl(APUSYS_DAPC_CON(base)));
}
