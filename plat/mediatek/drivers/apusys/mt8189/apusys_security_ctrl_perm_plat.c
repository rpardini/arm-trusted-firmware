...
int sec_get_dns(enum apusys_dev_type dev_type,
...
int sec_set_rv_dns(void)
{
	...
	ret = sec_get_dns(APUSYS_DEVICE_UP, SEC_LEVEL_SECURE,
		&sec_domain, &sec_ns);
	if (ret) {
		ERROR("%s fail\n", __func__);
		return ret;
	...
	ret = sec_get_dns(APUSYS_DEVICE_UP, SEC_LEVEL_NORMAL,
		&normal_domain, &normal_ns);
	if (ret) {
		ERROR("%s fail\n", __func__);
		return ret;
	}
